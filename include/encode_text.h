...
public:
    ...
    EncodeTextContext(std::string font_location)
    {
        ...
        {
            throw std::runtime_error{std::string("EncodeTextContext: Failed to init freetype: ") + std::string(FT_Error_String(ret))};
        }

        if ((ret = FT_New_Face(this->_library, font_location.c_str(), 0, &this->_face)) < 0)
        {
            throw std::runtime_error{std::string("EncodeTextContext: Failed to init font face: ") + std::string(FT_Error_String(ret))};
        }
        if ((ret = FT_Set_Char_Size(
                 this->_face, /* handle to face object           */
