...
class FileMuxingContext : public MuxingContext
{
public:
    FileMuxingContext()
    {
        unlink("/tmp/ngpfiles");
        mkdir("/tmp/ngpfiles", 0777);
    }

    void consume_packet(AVPacket *pkt)
    {
        std::string f = pkt->flags == AV_PKT_FLAG_KEY ? "k" : "d";
        std::string outfn = "/tmp/ngpfiles/" + std::to_string(av_gettime()) + f;
        tlog::info() << "FileMuxingContext::consume_packet: writing packet to " << outfn;
        std::ofstream myfile;
        myfile.open(outfn);
        myfile.write((const char *)pkt->data, pkt->size);
        myfile.close();
    }
};
