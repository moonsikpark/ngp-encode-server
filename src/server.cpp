...
void socket_manage_thread(std::string renderer, ThreadSafeQueue<std::unique_ptr<RenderedFrame>> &frame_queue, std::atomic<std::uint64_t> &frame_index, std::shared_ptr<VideoEncodingParams> veparams, CameraManager &cameramgr, std::atomic<bool> &shutdown_requested)
{
    while (!shutdown_requested)
    {
        ...
        std::thread _socket_client_thread(socket_client_thread, fd, std::ref(frame_queue), std::ref(frame_index), veparams, std::ref(cameramgr), std::ref(shutdown_requested));
...
void socket_main_thread(std::vector<std::string> renderers, ThreadSafeQueue<std::unique_ptr<RenderedFrame>> &frame_queue, std::atomic<std::uint64_t> &frame_index, std::shared_ptr<VideoEncodingParams> veparams, CameraManager &cameramgr, std::atomic<bool> &shutdown_requested)
{
    ...
    {
        threads.push_back(std::thread(socket_manage_thread, renderer, std::ref(frame_queue), std::ref(frame_index), veparams, std::ref(cameramgr), std::ref(shutdown_requested)));
    }
