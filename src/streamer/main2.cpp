...
shared_ptr<Client> createPeerConnection(const Configuration &config,
                                        ...
                                        string id, std::shared_ptr<AVCodecContextManager> ctxmgr);
...
void addToStream(shared_ptr<Client> client, bool isAddingVideo, std::shared_ptr<AVCodecContextManager> ctxmgr);
...
/// @param config Configuration
/// @param ws Websocket
void wsOnMessage(json message, Configuration config, shared_ptr<WebSocket> ws, std::shared_ptr<AVCodecContextManager> ctxmgr)
{
    ...
    {
        shared_ptr<Client> c = createPeerConnection(config, make_weak_ptr(ws), id, ctxmgr);
...
int stmain(std::shared_ptr<AVCodecContextManager> ctxmgr, std::atomic<bool> &shutdown_requested)
{
    ...
    while (!shutdown_requested)
    {
        ...
        case 0:
            ...
            rtp = reinterpret_cast<rtc::RtpHeader *>(pkt->data);
            rtp->setSsrc(ssrc);
                                        ...
                                        string id, std::shared_ptr<AVCodecContextManager> ctxmgr)
{
    ...
    client->video = addVideo(pc, 102, 1, "video-stream", "stream1", [id, wc = make_weak_ptr(client), ctxmgr]()
                             {
        MainThread.dispatch([wc, ctxmgr]() {
            if (auto c = wc.lock()) {
                addToStream(c, true, ctxmgr);
            }
        });
        cout << "Video from " << id << " opened" << endl; });

    client->audio = addAudio(pc, 111, 2, "audio-stream", "stream1", [id, wc = make_weak_ptr(client), ctxmgr]()
                             {
            MainThread.dispatch([wc, ctxmgr]() {
                if (auto c = wc.lock()) {
                    addToStream(c, false, ctxmgr);
                }
            });
...
/// Create stream
shared_ptr<Stream> createStream(const string h264Samples, const unsigned fps, const string opusSamples, std::shared_ptr<AVCodecContextManager> ctxmgr)
{
    // video source
    auto video = make_shared<H264FileParser>(h264Samples, fps, true, ctxmgr);
    // audio source
...
/// Start stream
void startStream(std::shared_ptr<AVCodecContextManager> ctxmgr)
{
    ...
    {
        stream = createStream(h264SamplesDirectory, 30, opusSamplesDirectory, ctxmgr);
        avStream = stream;
    }
    stream->start();
}
